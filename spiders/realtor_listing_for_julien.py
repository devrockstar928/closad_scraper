...
class RealtorListingForJulienSpider(scrapy.Spider):
    ...
    def parse_listing(self, response):
        ...
        if response.meta.get('baths'):
            ...
            baths = response.xpath('//div[@id="ldp-property-meta"]//li[@data-label="property-meta-bath"]/span/text()').extract_first()
        if baths and baths.strip() and baths.strip().isdigit():
            baths = int(baths.strip())
            l.add_value('baths', str(baths))
